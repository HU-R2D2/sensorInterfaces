...
namespace r2d2 {
    ...
    Length UltrasonicSensor::get_distance()
    {
        ...
        if ((std::chrono::high_resolution_clock::now() - lastReadingTimeStamp) > minimumReadingInterval) {
        ...
        while(!pin_get(echo)) {
            ...
            // if ((Clock::get_current_time() - waitingForPulseStartTimeStamp > echoTimeout)) {
                ...
                // return -1 * Length::CENTIMETER;
            // }
            if ((std::chrono::high_resolution_clock::now() - waitingForPulseStartTimeStamp) > echoTimeout) {
                // Return -1 centimeter (error)
                return -1 * Length::CENTIMETER;
            }
        }
        ...
        while(pin_get(echo)) {
            ...
            // if ((Clock::get_current_time() - signalSentTimeStamp) > echoTimeout) {
                ...
                // return -1 * Length::CENTIMETER;
            // }
            if ((std::chrono::high_resolution_clock::now() - signalSentTimeStamp) > echoTimeout) {
                // Return -1 centimeter (error)
                return -1 * Length::CENTIMETER;
            }
        }
        ...
        std::chrono::duration<long, std::micro> travelTime = echoReceivedTimeStamp - signalSentTimeStamp;
        ...
        if ((2 * Length::CENTIMETER) < distance && distance < (330 * Length::CENTIMETER)) {
            return distance;
        }
